// dsh-pocket-k 插件入口（单包单插件：手机扫码访问 DSH，全在这一个包里）
//
// 设置一级入口「手机访问」：
//   - 局域网二维码：自动显示（代理随插件启动）
...
import { createDshCookieProvider } from './dsh-auth.mjs';
...
import { lanAuthEnabled, setLanAuthEnabled, lanEnabled, setLanEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, publicBaseUrl, setPublicBaseUrl, clearPublicBaseUrl, cfMode, setCfMode, clearCfMode, cfApiToken, setCfApiToken, clearCfApiToken } from './settings.mjs';
import { fetchLatestUpstreamVersion } from './upstream.mjs';
...
/** 启动时异步拉取的上游最新 release tag（GitHub）；失败为 null。 */
let upstreamLatest = null;

// ---------- 访问密码（issue #13 + #18 + #33） ----------
// 公网与局域网**分开**：各自 8 位数字，存本机 $DSH_HOME/dsh-pocket-k/。
...
function newPin() {
  // SEC-2：Math.random() 非 CSPRNG（xorshift128+，少量输出即可恢复内部状态从而预测后续值），
  // 而 PIN 是公网/LAN 访问的唯一认证凭据——改用加密安全随机；[min, max) 均匀分布，8 位无模偏差。
  return String(randomInt(10_000_000, 100_000_000));
}

// --- 公网密码 ---
const tokenRel = join('dsh-pocket-k', 'token');
function tokenPath() {
...
const lanTokenRel = join('dsh-pocket-k', 'token-lan');
...
// ---------- 公网固定地址（named tunnel 入口） ----------
/** 该 Host 是否为用户登记的公网固定入口（Host 头与 publicBaseUrl 的 host 一致）。 */
function hostIsPublicBase(host) {
  const base = publicBaseUrl();
  if (!base) return false;
  try {
    return String(host ?? '').trim().toLowerCase() === new URL(base).host.toLowerCase();
  } catch { return false; }
}

// 固定域名入口的会话密钥：持久化文件（0600）——cookie 绑定它 → 登录态跨 dsh web
// 重启存活（本功能核心红利）；其余 Host 仍用进程级随机 key（issue #33：重启后重新输入）。
const sessionKeyRel = join('dsh-pocket-k', 'session-key');
let stableSessionKeyCache = null;
function stableSessionKey() {
  if (stableSessionKeyCache) return stableSessionKeyCache;
  const p = join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), sessionKeyRel);
  try {
    const k = readFileSync(p, 'utf8').trim();
    if (/^[0-9a-f]{64}$/.test(k)) { stableSessionKeyCache = k; return k; }
  } catch { /* 无文件 */ }
  ...
  stableSessionKeyCache = fresh;
  return fresh;
}

/** 按 Host 分发的访问密码：公网隧道/固定域名入口用公网密码，局域网用局域网密码。 */
function tokenForHost(host) {
  return /trycloudflare\.com$/i.test(String(host ?? '')) || hostIsPublicBase(host) ? getAccessToken() : getLanToken();
}
/**
...
const restartNoticeRel = join('dsh-pocket-k', 'restarted.json');
...
/** 执行更新：dsh plugin --profile <p> update dsh-pocket-k --latest -w（超时保护）。 */
function performUpdate(profile, { timeoutMs = 180_000 } = {}) {
  return new Promise((resolve) => {
    const child = spawn('dsh', ['plugin', '--profile', profile, 'update', 'dsh-pocket-k', '--latest', '-w'], {
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  // 每次启动异步检查上游最新 release（失败静默，不影响启动/设置页）
  void fetchLatestUpstreamVersion().then((v) => {
    if (v) upstreamLatest = v;
  }).catch(() => {});
  const dshPort = internals.dshPort ?? ctx.webServer?.port;
  if (!dshPort) {
    logger.error('dsh-pocket-k: webServer port unavailable — cannot start proxy | 拿不到 dsh web 端口，无法启动代理');
    return () => {};
  }
  ...
  if (isDesktop) {
    logger.info('dsh-pocket-k: DSH Desktop detected — update/restart disabled here | 检测到桌面端环境，更新/重启已关闭');
  }
  ...
  if (desktopAdvanced) {
    logger.warn('dsh-pocket-k: DSH Desktop advanced mode — phone access unsupported, injecting notice | 桌面端 advanced 模式：手机访问暂不支持，已注入提示');
  }

  const registry = createDeviceRegistry({ home: internals.home });

  // DSH 自身浏览器认证（dsh-client-connection）自愈：代理把 Host 改写成 loopback 后，
  // DSH 要求的是「按 loopback authority 命名并签名」的 cookie（浏览器在公网域名下的
  // 旧 cookie 与之不匹配 → `dsh web authentication required`）。这里用官方
  // connection.authenticatedUrl() 自动完成一次 token 交换并缓存 Set-Cookie，
  // 随后注入每个上游请求 —— 手机/公网/CF 反代都不再需要手工打开带 token 的 URL。
  const dshAuth = createDshCookieProvider({
  ...
  const service = internals.service ?? createPocketService({
    ...
    // t13：登录成功后铸造设备会话 token，cookie 存 token；sessionKey 仅作为旧路径回退。
    auth: {
      devices: registry,
      // sessionKey：进程级随机密钥——旧 cookie 派生回退（仅 auth.devices 缺席时生效）；新逻辑不用
      sessionKey: randomBytes(16).toString('hex'),
      // 固定域名入口例外：cookie 绑定持久化 session-key → 登录态跨重启存活（核心红利）；
      // 其余 Host 返回 undefined → 回退进程级 sessionKey
      sessionKeyFor: (host) => (hostIsPublicBase(host) ? stableSessionKey() : undefined),
      getToken: (host) => tokenForHost(host),
      isProtected: (host) => /trycloudflare\.com$/i.test(String(host ?? '')) || hostIsPublicBase(host) ? true : lanAuthEnabled(),
    },
    // 每次公网隧道就绪 → 轮换 8 位密码（旧密码/旧链接立即作废；用户自定义后不轮换）
    onTunnelReady: () => {
      const fresh = rotateAccessToken();
      logger.info('dsh-pocket-k: public access PIN refreshed | 公网访问密码已更新（自定义密码不受影响）');
      return fresh;
    },
  });
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    getPublicBase: () => publicBaseUrl(),
    setPublicBase: (value) => setPublicBaseUrl(value),
    clearPublicBase: () => clearPublicBaseUrl(),
    getCfMode: () => cfMode(),
    setCfMode: (mode) => setCfMode(mode),
    clearCfMode: () => clearCfMode(),
    getCfApiToken: () => cfApiToken(),
    setCfApiToken: (token) => setCfApiToken(token),
    clearCfApiToken: () => clearCfApiToken(),
    getUpstreamLatest: () => upstreamLatest,
  ...
  disposers.push(() => { void registry.dispose(); });

  // 代理随插件自动启动（局域网二维码开箱即用，零配置）
  void dshAuth.start();
  void service.startProxy().then((proxy) => {
    logger.info('dsh-pocket-k: proxy ready on :%d | 局域网代理已就绪', proxy.port);
    // 自动恢复上次开启的公网隧道（DSH 重启后 cloudflared 子进程被杀，issue #11）
    void service.restoreTunnelIfNeeded?.().catch(() => {});
  }).catch((err) => {
    logger.error('dsh-pocket-k: proxy start failed | 代理启动失败: %s', err?.message ?? err);
  });

  ctx.effect(() => async () => {
    for (const d of disposers.reverse()) { try { d(); } catch { /* 忽略 */ } }
    await service.dispose();
  }, 'dsh-pocket-k: stop proxy and tunnel');
}
