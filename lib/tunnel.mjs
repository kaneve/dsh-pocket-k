 ...
 * 优先：PATH 已有 → 直接用；否则用持久缓存（$DSH_HOME/dsh-pocket-k/cloudflared），
...
export async function resolveCloudflared({ home, onPhase = () => {}, signal } = {}) {
  ...
  const cacheDir = join(dshHome, 'dsh-pocket-k', 'bin');
  const { os, a, ext } = platformBinary();
      ...
      if (os === 'linux') {
        try {
          ...
          if (head.includes('@@HOMEBREW_PREFIX@@')) {
            ...
            console.warn('dsh-pocket-k: discarding unusable Homebrew-bottle cloudflared cache | 丢弃不可用的 Homebrew bottle 缓存，重新下载');
            continue;
          }
        } catch { /* 读失败按正常缓存处理 */ }
      }
...
export async function startQuickTunnel({ port, home, signal, onPhase = () => {} }) {
  ...
  child.on('error', (err) => {
    cleanup?.();
    onPhase?.('error');
    rejectErr?.(new Error(`cloudflared 启动失败：${err?.message ?? err}（可删除 $DSH_HOME/dsh-pocket-k/bin 缓存后重试）`));
  });
  onPhase('registering');
