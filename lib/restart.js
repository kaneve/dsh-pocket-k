// dsh-pocket-k 自重启：重新拉起启动本宿主的确切 dsh 调用（detached 交接），
...
export function restartHost({ handoffMs = 1500, internals = {} } = {}) {
  ...
  const logOut = join(tmpdir(), `dsh-pocket-k-restart-${stamp}.out.log`);
  const logErr = join(tmpdir(), `dsh-pocket-k-restart-${stamp}.err.log`);
