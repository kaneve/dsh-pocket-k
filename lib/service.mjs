// dsh-pocket-k 服务：在 dsh web 进程内跑改头代理 + 公网隧道
//
...
export function createPocketService({
  ...
  getLanIpOverride,
  /** 局域网访问总开关：true=绑 0.0.0.0（默认），false=只绑 127.0.0.1 */
  getLanEnabled,
  /** 代理注入 HTML 的内容（桌面端补丁等由 lib/index.js 传入；默认 randomUUID polyfill） */
  injectHtml,
  /** 访问令牌认证配置（issue #13）：{ getToken, isProtected }，传给代理 */
  auth,
  /** DSH 浏览器认证 cookie 自愈（见 lib/dsh-auth.mjs），传给代理 */
  dshAuth,
  /** 隧道就绪回调（lib/index.js 用它轮换公网密码） */
  onTunnelReady,
  /** 公网固定地址 getter：返回 https:// origin 或 null（named tunnel 固定入口） */
  getPublicBaseUrl,
  /** named tunnel 自动配置方式 getter：返回 'cli' | 'api' | null */
  getCfMode,
  /** Cloudflare API Token getter：返回 token 或 null（仅 api 模式） */
  getCfApiToken,
} = {}) {
  ...
  const autoStatePath = home ? join(home, 'dsh-pocket-k', 'tunnel-auto.json') : null;
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            port: p,
            host: (getLanEnabled?.() ?? true) ? '0.0.0.0' : '127.0.0.1',
          ...
          if (p !== port) {
            console.log(`dsh-pocket-k: port ${port} busy, proxy on ${p} | 端口 ${port} 被占用，代理改用 ${p}`);
          }
          break;
        } catch (err) {
    ...
    async startTunnel(opts = {}) {
      ...
      tunnelPromise = (async () => {
        try {
          const publicBase = getPublicBaseUrl?.() ?? null;
          const quick = opts?.quick === true;
          const result = publicBase && !quick
            ? await startNamed({ port: proxy.port, home, base: publicBase, mode: getCfMode?.() ?? null, apiToken: getCfApiToken?.() ?? null, signal: controller.signal, onPhase })
            : await startTunnel({ port: proxy.port, home, signal: controller.signal, onPhase });
          // 归一化：startTunnel 契约返回 {url, kill}（字符串也兼容）
          tunnel = typeof result === 'string' ? { url: result, kill: () => {} } : result;
          tunnelKind = publicBase && !quick ? 'fixed' : 'quick';
    ...
    async restoreTunnelIfNeeded() {
      ...
      try {
        ...
        console.log('dsh-pocket-k: public tunnel auto-restored | 已自动恢复公网隧道');
      } catch (err) {
        // 恢复失败保留标记（下次启动再试）；网络问题见 README 排障
        console.warn('dsh-pocket-k: tunnel auto-restore failed | 自动恢复隧道失败: %s', err?.message ?? err);
      }
    },

    /** 切换局域网访问总开关：重启代理以改变绑定地址（0.0.0.0 ↔ 127.0.0.1），公网隧道不受影响。 */
    async setLanAccessEnabled(on) {
      if (proxy) {
        const p = proxy;
        proxy = null;
    ...
    async status() {
      const lanEnabled = getLanEnabled?.() ?? true;
      const lan = await getLan();
      const proxyPort = proxy?.port ?? null;
      const publicBase = getPublicBaseUrl?.() ?? null;
      const lanUrl = lanEnabled && lan && proxyPort ? `http://${lan}:${proxyPort}` : null;
      ...
      return {
        ...
        lanCandidates,
        lanIpOverride,
        // 公网 QR/URL 只在实际隧道运行时显示；publicBase 仅作为配置保留在设置页，
        // 未开启时不显示二维码，避免误导为可访问状态。
        tunnelRunning: tunnel !== null,
        tunnelUrl: tunnel?.url ?? null,
        activeTunnelUrl: tunnel?.url ?? null,
        tunnelQr: await qrCached(tunnel?.url ?? null),
        tunnelState: { ...tunnelState },
        tunnelMode: tunnel ? tunnelKind : 'none',
        publicBase,
        dshPort,
      };
    },
