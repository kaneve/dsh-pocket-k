  ...
  return {
    ...
    async status() {
      ...
      return {
        ...
        lanCandidates,
        lanIpOverride,
        // 固定域名模式：公网二维码/URL 用用户登记的 origin（快速隧道没跑也有码）；
        // quick tunnel 降级为备用，tunnelRunning 仍只反映快速隧道进程本身
        tunnelRunning: tunnel !== null,
        tunnelUrl: tunnel?.url ?? publicBase ?? null,
        activeTunnelUrl: tunnel?.url ?? null,
        tunnelQr: await qrCached(tunnel?.url ?? publicBase ?? null),
