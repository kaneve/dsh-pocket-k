// dsh-pocket-k 核心：Host/Origin 改写反向代理
//
...
import zlib, { createGzip, createBrotliCompress, constants as zlibConstants, gunzipSync, inflateSync, brotliDecompressSync } from 'node:zlib';
import { createHash } from 'node:crypto';
import { applyDshCookie } from './dsh-auth.mjs';
import { WS_BRIDGE_PATH, WS_FALLBACK_MARK, WS_FALLBACK_SCRIPT, createWsBridge } from './ws-bridge.mjs';
...
const INJECT_MARK = 'data-dsh-pocket-k-polyfill="1"';
...
/**
 * 把（可能压缩的）响应体解成文本；未知编码或解压失败返回 null（调用方原样透传）。
 *
 * 为什么必须解压：真实浏览器一律带 Accept-Encoding，DSH 就回 gzip/br —— 不解压的话
 * HTML 注入分支会被整段跳过（历史 bug：polyfill / 桌面补丁 / 提示层对真实浏览器
 * 全部没生效）。解压后以**未压缩**形式返回（同时删掉 content-encoding）。
 */
function decodeBody(buf, headers) {
  ...
  try {
    ...
    if (enc === 'zstd' && typeof zlib.zstdDecompressSync === 'function') return zlib.zstdDecompressSync(buf).toString('utf8');
  } catch { return null; }
  return null;
}
...
export function advancedNoticeScript() {
  return `<script data-dsh-pocket-k-advanced-notice="1">!function(){try{var d=document.createElement('div');d.style.cssText='position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.55);color:#fff;font:15px/1.7 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;text-align:center;padding:24px';d.textContent='DSH 桌面端处于 advanced 模式，手机访问暂不支持。请在桌面端设置中切回 compatibility 模式后重启。| DSH Desktop is in advanced mode — phone access is not supported yet. Switch back to compatibility in the desktop app and restart.';document.documentElement.appendChild(d);}catch(e){}}();</script>`;
}
...
/** 设备会话 cookie 的 Set-Cookie 头（HttpOnly + SameSite=Lax + 30 天；不加 Secure 因 LAN 是 http）。 */
function sessionCookieHeader(token) {
  return `${TOKEN_COOKIE}=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${COOKIE_MAX_AGE}`;
}

/** 在响应头里追加 Set-Cookie（保留上游已有的 set-cookie，不覆盖）。 */
function appendSetCookie(headers, token) {
  if (!token) return headers;
  const next = { ...headers };
  const cookie = sessionCookieHeader(token);
  const existing = next['set-cookie'];
  if (!existing) next['set-cookie'] = cookie;
  else if (Array.isArray(existing)) next['set-cookie'] = [...existing, cookie];
  else next['set-cookie'] = [existing, cookie];
  return next;
}
...
/** 该 Host 是否受访问密码保护（公网隧道/固定域名入口经 auth.isProtected 注入；局域网 IP 直连免密码）。 */
function isProtectedHost(host, isProtected) {
  return isProtected ? isProtected(host) : /trycloudflare\.com$/i.test(String(host ?? ''));
}

// PWA 静态资源（manifest / favicon / 图标）**永远免密码放行**：
// 浏览器会在页面还没登录（甚至免凭证）时就抓取它们做"添加到主屏"与书签图标，
// 拿不到就刷控制台 401 报错、安装失败。DSH 直连（3080）本身也公开提供这些文件，
// 不含隐私内容，所以放行不会削弱门禁。
const PUBLIC_ASSET_PATHS = new Set(['/manifest.webmanifest', '/favicon.ico', '/favicon.svg']);

/** 请求是否为可公开的 PWA 静态资源（忽略 query）。 */
function isPublicAsset(reqUrl) {
  const path = String(reqUrl ?? '/').split('?')[0].toLowerCase();
  if (PUBLIC_ASSET_PATHS.has(path)) return true;
  return /^\/(apple-touch-icon|icon-\d+|favicon-\d+)[^/]*\.(png|svg|ico|webp)$/.test(path);
}
...
/**
 * 设备会话认证解析（t13）。
 * 优先按设备注册表校验独立会话 token；老 cookie（值 === 当前 PIN）视为有效并
 * 透明升级：铸一个新设备会话、返回 setCookie 供响应种下。无 devices 时回退旧
 * sessionKey 派生逻辑（测试/旧调用方兼容）。
 * @returns {{ok:boolean, setCookie?:string}}
 */
function sessionForRequest(req, host, token, auth) {
  const devices = auth?.devices;
  const cookieTok = parseCookies(req.headers.cookie)[TOKEN_COOKIE];
  if (devices) {
    if (cookieTok) {
      const session = devices.resolve(host, cookieTok);
      if (session) return { ok: true, deviceId: session.id };
    }
    const qTok = new URL(req.url ?? '/', 'http://x').searchParams.get('token');
    if (qTok) {
      const session = devices.resolve(host, qTok);
      if (session) return { ok: true, deviceId: session.id };
    }
    // 透明迁移：cookie 或 query 里仍是旧 PIN → 视为有效并铸新会话
    if (cookieTok === token || (qTok && qTok === token)) {
      const issued = devices.issue(host, req.headers['user-agent']);
      return { ok: true, setCookie: issued.token, deviceId: issued.id };
    }
    // 携带了设备 token 但未命中（已撤销/过期/伪造）→ 403；完全无 token 仍走常规 200 登录页/401 API
    return { ok: false, forbidden: Boolean(cookieTok || qTok) };
  }
  const sessionKey = auth?.sessionKeyFor?.(host) ?? auth?.sessionKey ?? null;
  const expected = cookieFor(token, sessionKey);
  if (cookieTok === expected) return { ok: true };
  const qTok = new URL(req.url ?? '/', 'http://x').searchParams.get('token');
  return { ok: qTok === expected };
}
...
// WebSocket 回落桥连上游时只允许带这几个头：握手头（upgrade / sec-websocket-*）必须由
// WS 客户端自己生成，原样转发会被 undici 直接拒绝（实测 upgrade、sec-websocket-key
// 会让连接失败）。其余入站头（content-length、accept-encoding 等）对上游 WS 无意义。
const BRIDGE_HEADER_ALLOWLIST = ['host', 'cookie', 'authorization', 'origin', 'referer', 'user-agent', 'accept-language'];

/** 从改写后的入站头里挑出给回落桥上游 WS 用的安全子集。 */
function pickBridgeHeaders(headers) {
  const out = {};
  for (const k of BRIDGE_HEADER_ALLOWLIST) {
 ...
 * @param {object} [opts.dshAuth]    DSH 浏览器认证 cookie 自愈（见 dsh-auth.mjs）：{ cookie, refresh, invalidate }
 * @param {boolean} [opts.wsFallback] WebSocket 回落通道（见 ws-bridge.mjs，默认开）：原生 WS 打不开时改走 HTTP 桥
 * @returns {Promise<{server:import('node:http').Server, close:()=>Promise<void>}>}
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {}, dshAuth = null, wsFallback = true } = {}) {
  const limiter = auth ? createRateLimiter(rateLimit ?? {}) : null;
  // WebSocket 回落通道（ws-bridge.mjs）：把浏览器的 WS 帧搬到普通 HTTP 上，
  // 给「只放行 HTTP、掐掉新建 TCP+TLS 的 wss」的网络兜底。注入关闭时不启用。
  const wsBridge = wsFallback === false || injectHtml === '' ? null : createWsBridge({ upstream, log });
  // 注入内容：调用方注入（polyfill 等）+ WS 回落脚本，各自按自己的标记判重
  const injectPieces = [];
  if (injectHtml) injectPieces.push([INJECT_MARK, injectHtml]);
  if (wsBridge) injectPieces.push([WS_FALLBACK_MARK, WS_FALLBACK_SCRIPT]);
  const server = createServer((req, res) => {
    // 透明迁移需要把新会话 cookie 附加到本次响应（请求本身先用旧 PIN 放行）
    let pendingSetCookie = null;
    // 访问令牌认证（issue #13 + #18 + #33 + #40 + t13）：局域网与公网按开关/来源要求密码
    if (auth) {
      const host = String(req.headers.host ?? '');
      const isPublic = /trycloudflare\.com$/i.test(host);
      // PWA 静态资源免门禁（见 isPublicAsset）；其余 Host 照常要求密码
      const protectedHost = isProtectedHost(host, auth.isProtected) && !isPublicAsset(req.url);
      const token = protectedHost ? (auth.getToken?.(host) ?? null) : null;
      // 会话密钥按 Host 解析（仅旧 sessionKey 回退路径使用；设备会话制下不再依赖）
      const sessionKey = auth.sessionKeyFor?.(host) ?? auth.sessionKey ?? null;
      if (protectedHost && token) {
        const ip = clientIp(req);
        // 登录提交：速率限制（issue #40）→ 校验密码 → 铸设备会话 token / 种持久 HttpOnly cookie → 回首页
        if (req.method === 'POST' && req.url?.startsWith('/pocket-login')) {
          ...
          req.on('end', () => {
            ...
            if (submitted === token) {
              limiter?.clear(ip);
              const issued = auth.devices?.issue(host, req.headers['user-agent']);
              const cookie = issued?.token ?? cookieFor(token, sessionKey);
              res.writeHead(302, appendSetCookie({
                location: '/',
                'cache-control': 'no-store',
              }, cookie));
              res.end();
              ...
              limiter?.record(ip);
              log?.(`dsh-pocket-k: login failed from ${ip} | 登录失败 IP: ${ip}`);
        ...
        const session = sessionForRequest(req, host, token, auth);
        if (!session.ok) {
          if (isHtmlRequest(req)) {
            // 锁定期间打开登录页也给提示（HTTP 200 + 锁定文案；429 语义留给 POST 拒绝）
            const rl = limiter?.status(ip) ?? { locked: false, retryAfter: 0 };
            const status = session.forbidden ? 403 : 200;
            res.writeHead(status, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
            res.end(loginPageHtml(rl.locked ? 'locked' : false, isPublic, rl.retryAfter));
          } else {
            // 携带已撤销/过期设备 token → 403；完全无凭证仍保持旧 401（兼容既有语义）
            const status = session.forbidden ? 403 : 401;
            res.writeHead(status, { 'content-type': 'application/json', 'cache-control': 'no-store' });
            res.end('{"error":"' + (session.forbidden ? 'forbidden' : 'unauthorized') + '"}');
          }
          return;
        }
        pendingSetCookie = session.setCookie ?? null;
      }
    }
    // DSH 自己的浏览器认证 cookie 由本代理代铸并注入（Host 已改写成 loopback，
    // 浏览器在公网域名下持有的旧 cookie 与 authority 不匹配 → 不做这步就吃 401）
    const headers = applyDshCookie(loopbackAuthority({ ...req.headers }, upstream), dshAuth?.cookie?.() ?? null);
    // WebSocket 回落通道端点：门禁已在上面校验过，这里复用刚算好的 loopback 权威与
    // 代铸 cookie，去连本机 dsh web 的 /api/remote.mux（HTTP 桥，见 ws-bridge.mjs）
    if (wsBridge) {
      req.__pocketBridgeHeaders = pickBridgeHeaders(headers);
      if (wsBridge.handle(req, res)) return;
    }
    const proxyReq = httpRequest(
      { host: upstream.host, port: upstream.port, method: req.method, path: req.url, headers, agent: false },
      (proxyRes) => {
        // 上游 401 = DSH 认证 cookie 失效/密钥轮换 → 丢弃缓存并后台重铸（下一次请求即恢复）
        if (proxyRes.statusCode === 401 && dshAuth) {
          dshAuth.invalidate?.();
        ...
        // 注入需要完整文本：压缩的 HTML（真实浏览器必带 Accept-Encoding）先解压再注入，
        // 注入后以未压缩形式返回（同时删掉 content-encoding）
        if (injectPieces.length && contentType.includes('text/html')) {
          const chunks = [];
          proxyRes.on('data', (c) => chunks.push(c));
          proxyRes.on('end', () => {
            const raw = Buffer.concat(chunks);
            const decoded = decodeBody(raw, proxyRes.headers);
            if (decoded === null) {
              // 未知编码 / 解压失败：原样透传（保底，绝不损坏页面）
              res.writeHead(proxyRes.statusCode ?? 200, appendSetCookie(proxyRes.headers, pendingSetCookie));
              res.end(raw);
              return;
            }
            let html = decoded;
            // 每段注入各自按标记判重：polyfill 已注入但回落脚本没有（或反之）时都能补齐
            for (const [mark, code] of injectPieces) {
              if (!html.includes(mark)) {
                html = html.replace(/<head[^>]*>/i, (m) => `${m}${code}`);
              }
            }
            ...
            res.writeHead(proxyRes.statusCode ?? 200, appendSetCookie(outHeaders, pendingSetCookie));
            res.end(out);
          });
        ...
        res.writeHead(proxyRes.statusCode ?? 502, appendSetCookie(proxyRes.headers, pendingSetCookie));
        proxyRes.pipe(res);
    ...
    proxyReq.on('error', (err) => {
      if (!res.headersSent) res.writeHead(502, appendSetCookie({ 'content-type': 'text/plain; charset=utf-8' }, pendingSetCookie));
      res.end(`dsh-pocket-k: 无法连接上游 dsh web（${upstream.host}:${upstream.port}）——先启动 dsh web | ${err.message}`);
    });
    req.pipe(proxyReq);
  });
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (auth) {
      ...
      if (token) {
        const session = sessionForRequest(req, host, token, auth);
        if (!session.ok) {
          socket.write(session.forbidden
            ? 'HTTP/1.1 403 Forbidden\r\nConnection: close\r\n\r\n'
            : 'HTTP/1.1 401 Unauthorized\r\nConnection: close\r\n\r\n');
          socket.destroy();
          return;
        }
        wsSetCookie = session.setCookie ?? null;
        wsDeviceId = session.deviceId ?? null;
      }
    }
    const headers = applyDshCookie(loopbackAuthority({ ...req.headers }, upstream), dshAuth?.cookie?.() ?? null);
    const proxyReq = httpRequest({
    ...
    proxyReq.on('upgrade', (proxyRes, proxySocket, proxyHead) => {
      ...
      if (wsSetCookie) raw.push(sessionCookieHeader(wsSetCookie));
      socket.write(`${raw.join('\r\n')}\r\n\r\n`);
      if (proxyHead?.length) socket.write(proxyHead);
      proxySocket.pipe(socket);
      socket.pipe(proxySocket);
      // 设备在线跟踪：握手成功后才算活跃 WS；关闭/错误时立即释放（见 devices.wsOpen/wsClose）
      let wsClosed = false;
      const closeWs = () => {
        if (wsClosed) return;
        wsClosed = true;
        if (wsDeviceId) auth?.devices?.wsClose(host, wsDeviceId);
      };
      if (wsDeviceId) auth?.devices?.wsOpen(host, wsDeviceId);
      ...
      const teardown = () => { closeWs(); try { proxySocket.destroy(); } catch {} try { socket.destroy(); } catch {} };
