// dsh-pocket-k 设备会话注册表（t13）：逐台撤销 + 持久化
//
...
//   - 定期 flush（默认 5s）批量落盘到 $DSH_HOME/dsh-pocket-k/devices.json (0600)；
...
export function devicesPath(home) {
  ...
  return join(dshHome, 'dsh-pocket-k', 'devices.json');
}
...
export function parseDeviceName(ua) {
  ...
  if (s.includes('Android')) {
    // Android UA 常见形态：...; Android 14; Pixel 8 Build/... 或 ; SM-S918B Build/...
    const m = s.match(/Android [\d.]+;\s*([^;)]+?)(?=\s+Build\/|\)|;)/);
    const model = m?.[1]?.trim();
    if (model && model.length >= 2 && !/^(K|unknown|Unspecified|wv|Build)$/i.test(model)) {
 ...
 *   resolve:(host:string, token:string)=>({ok:true,id:string}|null),
 *   wsOpen:(host:string, id:string)=>void,
 *   wsClose:(host:string, id:string)=>void,
...
export function createDeviceRegistry({
  ...
  const active = new Map(); // deviceId -> 活跃 WebSocket 连接数（仅内存，不持久化）
  let dirty = false;
        ...
        for (const [host, map] of Object.entries(raw)) {
          ...
          for (const [tokenId, rec] of Object.entries(map)) {
            ...
            inner.set(tokenId, {
              ...
              // 旧存档可能只有平台名（如 Android），用存的 UA 重新解析出机型，所有设备统一命名规则
              name: parseDeviceName(rec.ua),
  ...
  function gcHosts(now) {
    for (const [host, map] of hosts) {
      ...
      if (now - newest > ttlMs) {
        for (const rec of map.values()) dropActive(rec.id);
        hosts.delete(host);
        dirty = true;
      }
    }
  ...
  function evictGlobal() {
    while (totalDevices() > maxDevices) {
      ...
      dropActive(map.get(oldestId)?.id);
      map.delete(oldestId);
  ...
  function activeCount(id) {
    return active.get(String(id ?? '')) ?? 0;
  }

  function dropActive(id) {
  ...
  function flush() {
    ...
    try {
      ...
      console.warn(`dsh-pocket-k: devices.json flush failed | 设备注册表刷盘失败: ${err?.message ?? err}`);
    }
  ...
  return {
    ...
    /** 解析设备会话 token：校验通过返回 {ok:true,id}（供 WS 连接跟踪用），否则 null。 */
    resolve(host, token) {
    ...
    /** 记录一条设备 WebSocket 连接建立；同一设备可多条并发，计数累加。 */
    wsOpen(host, id) {
      const key = String(id ?? '');
      if (!key) return;
    ...
    /** 记录一条设备 WebSocket 连接断开；计数归零即视为离线。 */
    wsClose(host, id) {
      const key = String(id ?? '');
      if (!key) return;
    ...
    /** 列出设备；不传 host 则列出全部。online = 当前存在活跃 WebSocket 连接。 */
    list(host) {
      const out = [];
      if (host != null) {
        const map = hosts.get(String(host));
        if (map) {
          for (const rec of map.values()) {
            out.push({
              id: rec.id,
              name: rec.name,
              shortId: rec.id.slice(0, 6),
              online: activeCount(rec.id) > 0,
