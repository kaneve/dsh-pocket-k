#!/usr/bin/env node
// dsh-pocket-k — 把 DeepSeek Harness 装进你的口袋
//
// 用法：
//   dsh-pocket-k                 # 局域网模式：手机同一 WiFi 扫码访问
//   dsh-pocket-k --public        # 公网模式：cloudflared 隧道，人在外面也能访问
//   dsh-pocket-k --port 3081     # 自定义代理端口（默认 3081；dsh web 保持 3080）
//
...
function printHelp() {
  console.log(`dsh-pocket-k — 手机访问电脑上的 DeepSeek Harness

用法：
  dsh-pocket-k             局域网模式（手机同一 WiFi）
  dsh-pocket-k --public    公网模式（cloudflared 隧道，人在外面）
  dsh-pocket-k --port 3081 自定义代理端口
  dsh-pocket-k --help      帮助
...
async function main() {
  ...
  console.log(`✅ dsh-pocket-k 已就绪：手机扫码上面的二维码，看到的界面与电脑完全一致、实时同步。\n   按 Ctrl+C 停止。`);
  await new Promise(() => {});
}

main().catch((err) => {
  console.error(`❌ dsh-pocket-k: ${err?.message ?? err}`);
  process.exit(1);
});
