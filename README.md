...
<p align="center">
  内部版本 v1.14.0-k1（上游 shaobeichen/dsh-pocket；设置页每次启动时会检查 [GitHub Releases](https://github.com/shaobeichen/dsh-pocket/releases) 最新版并显示）
</p>
...
| 📱 移动端适配 | 窄屏自动变抽屉布局（移植 dsh-web-mobile **v2.4.1**，MIT）：侧栏抽屉 + **左右滑动手势**、会话全宽、状态栏安全区、触控优化、**会话删除菜单**、子代理 chip 触控、输入框键盘守卫、文件查看器兼容；本仓库保留 3 项本地定制（抽屉 z-index 抬到第三方浮层之上、explorer 显隐判定、紧凑状态栏标签） |
