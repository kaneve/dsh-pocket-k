...
| 📱 Mobile-adaptive layout | Narrow screens get a drawer layout automatically (ported from dsh-web-mobile **v2.4.1**, MIT): sidebar drawer + **swipe gestures**, full-width conversation, safe-area insets, touch optimizations, **session-delete menu**, subagent chip touch, composer keyboard guard, file-viewer compatibility; 3 local customizations are preserved (drawer z-index above third-party overlays, explorer visibility, compact stats labels); the phone chrome drops the explorer entry and the host's "open in app" dropdown (Explorer / Git Bash) |
